        ...
        <div className="pixel-card animate-pixel-fade-in">
          ...
          <div className="space-y-6">
            {passProgress.levels.map((level) => {
              const isLocked = level.level > passProgress.currentLevel;
              const isUnlocked = level.level <= passProgress.currentLevel;
              const isClaimed = passProgress.claimedRewards.includes(level.level);
              
              return (
                <div 
                  key={level.level}
                  className={`relative flex flex-col md:flex-row gap-4 items-center p-4 rounded-lg
                    ${isLocked ? 'bg-game-darkPurple/30 text-white/50' : 'bg-game-darkPurple'}
                ...
                >
                  ...
                  <div className={`absolute top-0 left-1/2 md:left-auto md:top-1/2 transform -translate-y-1/2 ${level.level % 2 === 0 ? 'md:right-0 md:translate-x-1/2' : 'md:left-0 md:-translate-x-1/2'} -translate-x-1/2 w-8 h-8 rounded-full flex items-center justify-center
                    ${isLocked ? 'bg-game-darkPurple text-white/50' : 'bg-game-yellow text-game-darkPurple'}`}
                  >
                  ...
                  <div className={`flex-1 ${level.level % 2 === 0 ? 'md:text-right' : ''}`}>
                    <h3 className={`font-pixel ${isLocked ? 'text-white/70' : 'text-white'}`}>
                      {level.reward.name}
                    </h3>
                    <p className={`text-sm ${isLocked ? 'text-white/50' : 'text-white/70'}`}>
